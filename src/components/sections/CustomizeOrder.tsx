...
const shirtTypes = [
 { id: 'short-sleeve', name: 'Manga Corta', imgSrc: '/camisetas/over1.png', hint: 'shortsleeve shirt', price: 20 },
 { id: 'long-sleeve', name: 'Manga Larga', imgSrc: '/camisetas/over2.png', hint: 'longsleeve shirt', price: 25 },
];
...
const designs = [
  ...
  { id: 'design2', name: 'Patrón Gráfico Urbano', imgSrc: '/disenios/astro.png', hint: 'graphic pattern', priceModifier: 5 },
  { id: 'design3', name: 'Arte Minimalista Elegante', imgSrc: '/disenios/caballopunk.png', hint: 'minimalist art', priceModifier: 3 },
  { id: 'design4', name: 'Logo Vintage Clásico', imgSrc: '/disenios/caballopunk.png', hint: 'vintage logo', priceModifier: 4 },
  { id: 'design5', name: 'Ilustración Naturaleza Fresca', imgSrc: '/disenios/caballopunk.png', hint: 'nature illustration', priceModifier: 6 },
];
...
export default function CustomizeOrder() {
  ...
  const handleAddToCart = () => {
    ...
    const itemToAdd = {
      // Use the actual AI generated image URL if available and AI design is selected
      imageSrc: selectedDesignId === 'ai' && aiGeneratedImageUrl ? aiGeneratedImageUrl : designForCart.imgSrc,
      shirtType: { id: shirtType.id, name: shirtType.name }, // Removed imgSrc from shirtType object as it's now handled by the main imageSrc
      color: { id: color.id, name: color.name, hex: color.hex },
      // Keep designForCart as it contains the design ID, name, and original placeholder/premade imgSrc
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-12 md:mb-16">
          ...
          <div className="flex flex-wrap gap-4 md:gap-6 justify-center">
            {colors.map((color) => ( 
              <button // Added outline for cyan border
                key={color.id}
                ...
                className={`w-12 h-12 md:w-16 md:h-16 rounded-full cursor-pointer transition-all duration-200 ease-in-out transform hover:scale-110 shadow-md ${color.twClass} ${color.borderClass || ''} ${
                  selectedColorId === color.id ? 'ring-4 ring-offset-2 ring-accent ring-offset-background' : 'ring-1 ring-border'
                    ...
                    style={{ backgroundColor: color.hex, outline: `1px solid var(--accent)` }} // Added outline for cyan border
                >
